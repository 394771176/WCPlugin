...
- (NSArray *)sortedArrayWithKeys:(NSArray *)keys ascendings:(NSArray*)ascendings {
    ...
    for (int i = 0; i < keys.count; i ++) {
        NSSortDescriptor *sort = [NSSortDescriptor sortDescriptorWithKey:[keys safeObjectAtIndex:i] ascending:[[ascendings safeObjectAtIndex:i] boolValue]];
...
+ (BOOL)validArrayFromDict:(NSDictionary *)dict forKey:(NSString *)key
{
    ...
    return [NSDictionary validDict:dict] && [self validArray:[dict objectForKey:key]];
}
...
- (id)getRandomObj
{
    if (self.count) {
        ...
        return [self safeObjectAtIndex:randomIndex];
    }
    return nil;
}
...
@implementation NSMutableSet (Utils)
